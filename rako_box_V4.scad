...
// Custom Length (used if standard dimensions are disabled)
custom_length = 300; 
// Custom Width (used if standard dimensions are disabled)
custom_width = 200; 
// Custom Height (used if standard dimensions are disabled)
custom_height = 117; 
                ...
                intersection() {
                    ...
                    union() {
                        if (support_triangles > 0) {
                            ...
                            for (i = [1 : support_triangles]) {
                                translate([-eps, i * (width / (support_triangles + 1)) - rib_w/2, 0])
                                    cube([length + 2*eps, rib_w, height]);
                            }
                        }
                        if (support_corners) {
...
if (part == "box") {
    ...
    color(box_color) rako_box();
    translate([0, width + 50, 0]) color(box_color) rako_lid();
} else if (part == "assemble") {
    color(box_color) rako_box();
    // Render the lid on top, slightly transparent so the snap-fit can be inspected
    translate([0, 0, height]) color(box_color, 0.85) rako_lid();
}
